...
def train_model(hidden_layer_sizes,max_iter,activation,solver,alpha,learning_rate,X_res, y_res , x_test, y_test):
    ...
    # show the results
    roc_auc, F1,recall,precision=confusion_AUC_Return(x_test,y_test,MLP_model)
    return roc_auc, F1,recall,precision
    #     print('AUC: {:.2f} | F1: {:.2f} | recall: {:.2f} | precision: {:.2f}'.format(roc_auc, F1,recall,precision))

#####################################################################################################

def main(hidden_layer_sizes,max_iter,activation,solver,alpha,learning_rate,file_num,model_name):   
    
    X_res, y_res , x_test, y_test=return_train_test_set(df_name='data_final.h5',key='data_final')    

    roc_auc, F1,recall,precision=train_model (hidden_layer_sizes,max_iter,activation,solver,alpha,learning_rate,X_res, y_res , x_test, y_test)
    
    data='{},{},{},{},{},{},{:.3f},{:.3f},{:.3f},{:.3f}'.format(hidden_layer_sizes,max_iter,activation,solver,alpha,learning_rate,roc_auc, F1,recall,precision)
...
if __name__ == "__main__":
    ...
    hidden_layer_sizes=tuple([int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3])])
    max_iter=int(sys.argv[4])
    activation=sys.argv[5]
    solver=sys.argv[6]
    alpha=float(sys.argv[7])
    learning_rate=sys.argv[8]
    file_num=int(sys.argv[9])
    model_name='ANN_3level/'

    main(hidden_layer_sizes,max_iter,activation,solver,alpha,learning_rate,file_num,model_name)
