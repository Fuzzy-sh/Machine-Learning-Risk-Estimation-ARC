...
def train_model( C,gamma,kernel,X_res, y_res , x_test, y_test):
     # define the model with hyper parameters
    svm_model = SVC(C= C,gamma= gamma,kernel= kernel)
    svm_model.fit(X_res, y_res, )

    # show the results
    roc_auc, F1,recall,precision=confusion_AUC_Return(x_test,y_test,svm_model)
    return roc_auc, F1,recall,precision
    #     print('AUC: {:.2f} | F1: {:.2f} | recall: {:.2f} | precision: {:.2f}'.format(roc_auc, F1,recall,precision))

#####################################################################################################

def main(C,gamma,kernel,file_num,model_name):   
    
    X_res, y_res , x_test, y_test=return_train_test_set(df_name='data_final.h5',key='data_final')    

    roc_auc, F1,recall,precision=train_model (C,gamma,kernel,X_res, y_res , x_test, y_test)
    
    data='{},{},{},{:.3f},{:.3f},{:.3f},{:.3f}'.format(C,gamma,kernel,roc_auc, F1,recall,precision)
...
if __name__ == "__main__":
    ...
    main(C,gamma,kernel,file_num,model_name)